...
import { Download, ExternalLink, Link2, Loader2, LockKeyhole, Upload, CheckCircle2, AlertTriangle } from 'lucide-react'
...
type LinkOutcome = { ok: boolean; text: string }
...
interface Props {
  ...
  isRtl: boolean
  disabled?: boolean
  /** Server answer after attaching: the fresh project row + the full BOQ list. */
  onAttached: (project: FurnProject, boqFiles: FileRef[]) => void
}
...
export function FurnFileActions({ project, notes, boqFiles, isRtl, disabled, onAttached }: Props) {
  ...
  const [outcome, setOutcome] = useState<Record<string, LinkOutcome>>({})
  ...
  async function fetchLink(url: string) {
    ...
    try {
      ...
      if (!res.ok) { const text = j.error || (isRtl ? 'تعذّر جلب الرابط' : 'Could not fetch the link'); setOutcome((o) => ({ ...o, [url]: { ok: false, text } })); toast.error(text, { duration: 10000 }); return }
      if (j.status !== 'files') {
        const text = j.message || (isRtl ? 'تعذّر جلب الرابط' : 'Could not fetch the link')
        setOutcome((o) => ({ ...o, [url]: { ok: false, text } }))
        toast.error(text, { duration: 12000 })
        return
      }
      onAttached(j.project as FurnProject, (j.boqFiles || []) as FileRef[])
      const text = `${isRtl ? `جُلب ${j.files.length} ملف من ${j.provider}: ` : `Fetched ${j.files.length} file(s) from ${j.provider}: `}${summary(j.added, j.demoted, isRtl)}${j.notices?.length ? ` — ${j.notices.slice(0, 3).join(' · ')}` : ''}`
      setOutcome((o) => ({ ...o, [url]: { ok: true, text } }))
      toast.success(`${text}. ${isRtl ? 'اضغط «إعادة المحاولة» للقراءة.' : 'Press “Retry” to read.'}`, { duration: 12000 })
      setManualUrl('')
    } catch (e) {
  ...
  const LinkRow = ({ l }: { l: DiscoveredLink }) => {
    const o = outcome[l.url]
    return (
      <div className="p-2 rounded border bg-muted/30 text-sm space-y-1">
        <div className="flex flex-wrap items-center gap-2">
          <span className="truncate flex-1 min-w-0" title={l.url}>
            {l.label}
            <span className="text-[11px] text-muted-foreground ms-1">({l.kind})</span>
          </span>
          <Button size="sm" variant="outline" className="gap-1 h-7" disabled={disabled || !!busy} onClick={() => fetchLink(l.url)}>
            {busy === l.url ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Download className="w-3.5 h-3.5" />}
            {o?.ok ? (isRtl ? 'جلب مرة أخرى' : 'Fetch again') : (isRtl ? 'جلب' : 'Fetch')}
          </Button>
        ...
        {o ? (
          <p className={`text-[12px] flex items-start gap-1 ${o.ok ? 'text-emerald-700' : 'text-amber-800'}`}>
            {o.ok ? <CheckCircle2 className="w-3.5 h-3.5 mt-0.5 shrink-0" /> : <AlertTriangle className="w-3.5 h-3.5 mt-0.5 shrink-0" />}
            <span>{o.text}{!o.ok && (isRtl ? ' — بعد التحميل استخدم «أضف ملفات» أدناه.' : ' — after downloading use “Add files” below.')}</span>
          </p>
        ) : l.likelyLogin ? (
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="rounded-lg border p-3 space-y-2">
        ...
        {manualUrl.trim() && outcome[manualUrl.trim()] && !links.some((l) => l.url === manualUrl.trim()) && (
          <p className={`text-[12px] ${outcome[manualUrl.trim()].ok ? 'text-emerald-700' : 'text-amber-800'}`}>{outcome[manualUrl.trim()].text}</p>
        )}
