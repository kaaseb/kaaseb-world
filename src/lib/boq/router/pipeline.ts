...
13. COMPOSITE UNITS WITH A STONE TOP (real case: a "Vanity Top" package listed under INTERIOR ARCHITECTURAL WOOD WORK): a row such as "Vanity counter unit; type ST-02+WD-01; dimension 1425 x 600 x 525mm; to fit 1 basin; as Drg. AID-00-8022" is a stone counter TOP (code ST-xx) sitting on a joinery carcass (WD-xx). The client sent this package to a STONE supplier: the stone top IS OURS. Emit it as an item — description = "Vanity top ST-02" (the stone code is the title), details = "كود: ST-02 – 1425 x 600 mm (top) – 1 basin – Drg. AID-00-8022 – الخزانة الخشبية WD-01 خارج نطاقنا", quantity/unit as written (Nr), department_match = the covered department you judge most likely for a counter top (the material is defined in a legend/drawing you cannot see — do NOT invent it; the deterministic gate flags the row for the team to confirm). A row that is joinery ONLY (WD-xx with no ST/stone code) is not ours.
14. THE PACKAGE NAME MATTERS: the file name, "SUBCONTRACT PACKAGE:" line, or subject tells you what the client is buying from us ("Vanity Top", "Stone Cladding", "Marble Flooring"). Read every row of such a package as a candidate for that stone scope, even when the section heading says WOOD WORK / JOINERY / FINISHES.
